...
def register():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(user)
        db.session.commit()
        flash('Congratulations, yor are now a registered user!')
        return redirect(url_for('login'))
...
def unfollow(username):
    ...
    if form.validate_on_submit():
        user = User.query.filter_by(username=username).first()
        if user is None:
            flash('User {} does not exists'.format(username))
            return redirect(url_for('index'))
...
def reset_password_request():
    ...
    if form.validate_on_submit():
        ...
        flash('Check your email for the instruction to reset the password')
        return redirect(url_for('login'))
